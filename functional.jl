...
type FuncHicken2011 <: AbstractFunctional
end
function call{Tmsh, Tsol, Tres, Tdim}(obj::FuncHicken2011,
                                      mesh::AbstractMesh{Tmsh},
                                      sbp::AbstractSBP,
                                      eqn::EllipticData{Tsol, Tres, Tdim},
                                      opts,
  ...
  for iBC = 1:mesh.numBC
    ...
    if isDirichlet(bc_func)
      for f = indx0:indx1
        val_j = zeros(Tsol, mesh.numNodesPerFace)
        # we only consider one side, x ∈ [0,1], y=0 
        xy = sview(mesh.coords_bndry, :, :, f)
        y = sview(xy, 2, :)
        if maximum(abs(y[:])) > 1e-13
          continue
        end
        ...
        # contribution from Dirichlet term
        for n = 1:mesh.numNodesPerFace
          val_j[n] -= (nrm[n,1] * Fv_face[1, 1, n] + nrm[n,2] * Fv_face[2,1,n])
        end
        ...
        for n = 1 : mesh.numNodesPerFace
          for i = 1 : mesh.numNodesPerFace
            val_j[n] += pMat[1, n, i] * dq[1, i] 
          end
        end
        # do the integral
        # beta = Array(Float64, mesh.numNodesPerFace)
        for n = 1 : mesh.numNodesPerFace
          xy = sview(mesh.coords_bndry, :, n, f)
          ex = exp(xy[1])
          beta = -pi*pi *(ex - 1) * (e - ex) / ((e-1)*(e - 1))
          val[2] += wface[n] * beta * val_j[n]
        end
        val[2] *= relax_coef
      end
